...
import { CallSendFunction, DeployCallbackFunction, EthProviderInterface, SendOptions } from "@saturn-chain/dlt-tx-data-functions";
...
export function ganacheProvider() {
  return Ganache.provider({
    ...
    chain: {vmErrorsOnRPCResponse:true, allowUnlimitedInitCodeSize: true, allowUnlimitedContractSize: true}, 
...
export async function getNewWallet(web3: Web3, name: string, reset: boolean=false): Promise<EthProviderInterface> {
  if (reset) addressUsed = 0;
  const accounts = await web3.eth.getAccounts();
  const address = accounts[addressUsed++]; map(address, name);
...
function traceEventLogActual( prefix: string|undefined, log: EventData,) {
  ...
  if (mapAddress.has(log.address)) log.address = mapAddress.get(log.address)||"undefined";
...
export function receipientInfo(account?: string, bic?: string, iban?: string) {
  if (account) return {account, bic: Buffer.alloc(11), iban: Buffer.alloc(32)};
  if (bic || iban) {
    let buff = Buffer.alloc(32);
    Buffer.from(iban||"").copy(buff, 0);
    const r = {
      account: ZeroAddress, 
      bic: bic? Buffer.from(`${bic}           `.slice(0, 11)): Buffer.alloc(11), 
      iban: buff
    };
...
export class DLTInterfaceEx extends Web3FunctionProvider {
  ...
  send(options: SendOptions = {maxGas: 5_000_000}): CallSendFunction {
    ...
    if (typeof options.amount === "string" && (options.amount as string).startsWith("0x")) options.amount = BigInt(options.amount);
...
export function cleanEvent(ev: EventData): EventData {
  ...
  res.returnValues = {};
  if (ev.returnValues) {
    for (const key of Object.keys(ev.returnValues) ) {
      if (Number.isNaN(Number.parseInt(key)))
        res.returnValues[key] = ev.returnValues[key];
    }
  }
  return res;
}
